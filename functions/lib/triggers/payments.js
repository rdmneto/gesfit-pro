...
export const asaasWebhook = onRequest({ secrets: [asaasApiKey, asaasWebhookSecret] }, async (request, response) => {
    ...
    try {
        ...
        const event = await gateway.parseWebhook(request.rawBody, normalizeHeaders(request.headers));
